...
#define UE_PRIVATE_LOG(Condition, CategoryConst, Category, VerbosityStr, Format, ...) \
	{ \
		auto Verbosity = ParseLogVerbosityFromString(TEXT(#VerbosityStr)); \
		UE_VALIDATE_FORMAT_STRING(Format, ##__VA_ARGS__); \
		FMsg::Logf_InternalImpl(__FILE__, __LINE__, TEXT(#Category), Verbosity, Format, __VA_ARGS__); \
	}
