...
public:
     ...
     */
    FQuat(struct FVector Axis, float AngleRad);
...
FORCEINLINE void FQuat::ToAxisAndAngle(struct FVector& Axis, float& Angle) const
{
    Angle = GetAngle();
...
FORCEINLINE FVector FQuat::RotateVector(struct FVector V) const
{
#if WITH_DIRECTXMATH
    FVector Result;
    ...
    const struct FVector Q(X, Y, Z);
    const struct FVector T = 2.f * FVector::CrossProduct(Q, V);
    const struct FVector Result = V + (W * T) + FVector::CrossProduct(Q, T);
    return Result;
#endif
}

FORCEINLINE FVector FQuat::UnrotateVector(struct FVector V) const
{
#if WITH_DIRECTXMATH
    FVector Result;
    ...
    const struct FVector Q(-X, -Y, -Z); // Inverse
    const struct FVector T = 2.f * FVector::CrossProduct(Q, V);
    const struct FVector Result = V + (W * T) + FVector::CrossProduct(Q, T);
    return Result;
#endif
